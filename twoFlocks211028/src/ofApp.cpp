    ...
    while (osc.hasWaitingMessages()) {
        ...
        if (message.getAddress() == "/minibee/data") { ///corfu
            if (message.getArgAsFloat(0) == 1) {
                ...
                alignData1 = ofMap(datax1, 0.46, 0.55, 0.1, 3);
                cohesionData1 = ofMap(datay1, 0.45, 0.54, -0.35, 1.5);
                separationData1 = ofMap(dataz1, 0.45, 0.55, 2, 8);
        ...
        if (message.getAddress() == "/minibee/data") {  //athens
            if (message.getArgAsFloat(0) == 1) {
                datax2 = message.getArgAsFloat(1);
                datay2 = message.getArgAsFloat(2);
                dataz2 = message.getArgAsFloat(3);
                alignData2 = ofMap(datax2, 0.45, 0.55, 0.1, 3);
                cohesionData2 = ofMap(datay2, 0.45, 0.54, -0.35, 1);
                separationData2 = ofMap(dataz2, 0.46, 0.55, 2, 8);
                ...
                alignData3 = ofMap(datax3, 0.45, 0.55, 0.1, 3);
                cohesionData3 = ofMap(datay3, 0.45, 0.54, -0.35, 1);
                separationData3 = ofMap(dataz3, 0.46, 0.55, 2, 8);
        ...
        for (int i = 0; i < flock.size(); i++) {
            ...
            flock[i].almult = alignData1 * freezeB;
            flock[i].comult = cohesionData1;
            flock[i].semult = separationData1;
            flock[i].accData.y = ofMap(datax, 0.45, 0.55, 1, -1);
            flock[i].accData.x = ofMap(datay, 0.45, 0.55, 1, -1);

        }
        for (int i = 0; i < flock2.size(); i++) {
            flock2[i].vell.x = velData2.x;// * freezeA;
            flock2[i].vell.y = velData2.y;// * freezeA;
            flock2[i].almult = alignData3 * freezeD;
            flock2[i].comult = cohesionData3;
            flock2[i].semult = separationData3;
            flock2[i].accData.y = ofMap(datax2, 0.45, 0.55, 1, -1);
            flock2[i].accData.x = ofMap(datay2, 0.45, 0.55, 1, -1);
        }
    }
}
//--------------------------------------------------------------
void ofApp::draw(){
    for (int i = 0; i < flock.size(); i++) {
        flock[i].update();
        flock[i].drawLines(i, graphicsBoo, color, flock);
        //cout<<velData.x<<endl;
    ...
    for (int i = 0; i < flock2.size(); i++) {
        ...
        // flock2[i].drawLines(i, graphicsBoo, color, flock2);
